...
public class ProductCacher {
    ...
    public ProductEntity readProductInfo(String detailUrl) throws Exception {
        ...
        if (StringUtils.isEmpty(productEntity.getUnitContent())) {
            ...
            String parten2 = "([0-9]+[\\u7247]{1})|([0-9]+[\\u7c92]{1})|([0-9]+[\\u514b]{1})|([0-9]+[\\u652f]{1})|([0-9]+[g]{1})|([0-9]+[m]{1}[l]{1})|([0-9]+[m]{1}[L]{1})|([0-9]+[m]{1}[l]{1})|([0-9]+[k]{1}[g]{1})|([0-9]+[K]{1}[G]{1})|([0-9]+[L]{1})";
            Pattern pattern = Pattern.compile(parten);
            Matcher matcher = pattern.matcher(productEntity.getProductChineseName());
            if (matcher.find()) {
        ...
        if (nodeList != null && nodeList.size() > 0) {
            ...
            while (iterator.hasMoreNodes()) {
                ...
                if (index2 == 1) {
                    productEntity.setProductEnglishName(StringUtils.trim(node4.toPlainTextString()));
                    index2++;
                }
                if (index2 == 3) {
                    productEntity.setBrandEnglishName(StringUtils.trim(node4.toPlainTextString()));
                    index2++;
                }
